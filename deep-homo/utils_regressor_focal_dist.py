...
class RotNetDataGenerator(object):
    ...
    def generate(self, image_path,labels):
        # Infinite loop
        while 1:
            ...
            for i in range(imax):
                ...
                # Generate data
                # try:
                X, y = self.__data_generation(image_path_temp,labels_temp)
    ...
    def __data_generation(self, image_path_temp,labels):
        ...
        for index, current_path in enumerate(image_path_temp):
            ...
            batch_label_u0.append(labels[index][1])
            batch_label_v0.append(labels[index][2])
            batch_label_baseline.append(labels[index][3])
            batch_label_x.append(labels[index][4])
            batch_label_y.append(labels[index][5])
            batch_label_z.append(labels[index][6])
            batch_label_pitch.append(labels[index][7])
            batch_label_xWorld.append(labels[index][8])
            batch_label_yWorld.append(labels[index][9])
            batch_label_zWorld.append(labels[index][10])
        batch_label_focal = np.array(batch_label_focal)
        batch_label_u0 = np.array(batch_label_u0)
        batch_label_v0 = np.array(batch_label_v0)
        batch_label_baseline = np.array(batch_label_baseline)
        batch_label_x = np.array(batch_label_x)
        batch_label_y = np.array(batch_label_y)
        batch_label_z = np.array(batch_label_z)
        batch_label_pitch = np.array(batch_label_pitch)
        batch_label_xWorld = np.array(batch_label_xWorld)
        batch_label_yWorld = np.array(batch_label_yWorld)
        batch_label_zWorld = np.array(batch_label_zWorld)
        # preprocess input images
        if self.preprocess_func:
            batch_x = self.preprocess_func(batch_x)

        return batch_x, {'fx': batch_label_focal, 'fy': batch_label_focal, 'u0': batch_label_u0, 'v0': batch_label_v0, 'baseline': batch_label_baseline, 'x': batch_label_x, 'y': batch_label_y, 'z': batch_label_z, 'pitch': batch_label_pitch, 'xWorld': batch_label_xWorld, 'yWorld': batch_label_yWorld, 'zWorld': batch_label_zWorld}
